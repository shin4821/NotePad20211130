...
Down::Down(NotePadForm* notePadForm)
...
void Down::KeyDown() {
	...
	//2. 아랫쪽 방향키 눌렀을 때
	  ...
	  if (currentY + 1 < length) {
		  ...
		  //2.3.2. y+1좌표에서 현재 너비보다 작거나 같을 동안 반복한다.
		  currentY++;
		  this->notePadForm->location->caret->x = 0;
		  ...
		  if (i < length) {
			  ...
			  //this->notePadForm->location->CalculateX(chaCharacter);
			  this->notePadForm->getTextSize->CalculateX(chaCharacter);
			  downWidth = this->notePadForm->location->caret->GetX();



			  if (downWidth < width && i < length) {
				  i = 1;
				  while (downWidth < width && i < length) { // downWidth가 더 커진 상태에서 나오게된다.

					  previousDownWidth = downWidth;
				  ...
				  if (downWidth > width) {
					  downWidth = previousDownWidth;
					  i--;
				  }
			  }
			  else if(downWidth != width){
				  downWidth = previousDownWidth;
			  }
			  else if (downWidth == width) {
				  i = 1;
		  ...
		  if (downWidth < width && i<length) {
			  ...
			  //this->notePadForm->location->CalculateX(chaCharacter);
			  //addWidth = this->notePadForm->location->caret->GetX();
			  //difference = addWidth - downWidth;

			  //2. y+1좌표 총 x너비 + 문자 반 너비보다 현재 너비가 더 크면, y+1좌표의 x좌표를 하나 더해준다.
			  if (downWidth + (addWidth / 2) <= width) {
				  i++;
				  currentX = i; //좌표변경
				  downWidth = downWidth + addWidth; //너비변경
			  }
		  }

		  //2.3.4. currentX, currentY를 현재 current로 설정한다.
